...
(defn impersonation-authorized-wrapper
  ...
  (fn is-impersonatable?
    ...
    (and (or (nil? impersonator)
             ((object-type->verb->impersonatable? item) verb))
...
(defn create-impersonation-middleware
  ...
  Uses the authorized-impersonators set argument to verify impersonation authorization."
  [authorized-impersonators]
  (fn impersonation-middleware [h]
    (if-let [impersonators-set (some-> authorized-impersonators seq set)]
      (fn impersonation-wrapper [{user :authorization/user :as req}]
        (if-let [impersonated-principal (get-in req [:headers "x-cook-impersonate"])]
          (let [[impersonated-user] (clojure.string/split impersonated-principal #"[/@]" 2)]
